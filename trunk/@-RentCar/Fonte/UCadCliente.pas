 ...
 Begin
    ...
    if frmRentCarPrincipal.tipo = 'PJ' then
     ...
     Begin
      ZQCliente.Close;
      ZQCliente.SQL.Clear;
      ZQCliente.SQL.Add('Select rentcar_pessoa.Pes_Id as Codigo,rentcar_pesfis.PesFis_nome as Nome, rentcar_pesfis.pesfis_Sexo as Sexo, rentcar_pessoa.pes_TelRes as TelRes, rentcar_pesfis.pesfis_RG as RG, rentcar_pesfis.pesfis_CPF as CPF');
      ZQCliente.SQL.Add('from rentcar_pessoa');
      ZQCliente.SQL.Add('INNER join rentcar_pesfis on rentcar_pesfis.RentCar_Pessoa_Pes_id = rentcar_pessoa.Pes_id');
      ZQCliente.SQL.Add('where rentcar_pessoa.pes_id = rentcar_pesfis.RentCar_Pessoa_Pes_id and rentcar_pesfis.PesFis_Tipo = "F"');
      ZQCliente.Open;
      ZQCliente.FieldByName('Pes_Id').DisplayLabel := 'Codigo';
     end;
     ZQCliente.Open;
...
begin
  ...
  Begin
    ShowMessage('Nome é um campo obrigratório');
    DBENome.SetFocus;
  end else
  if (DBECPF.Text = '') then
  Begin
    ShowMessage('CPF é um campo obrigratório');
    DBECPF.SetFocus;
  end else
  if (DBERG.Text = '') then
  Begin
    ShowMessage('RG é um campo obrigratório');
    DBERG.SetFocus;
  end else
  if (dbrgSexo.ItemIndex = -1) then
  Begin
    ShowMessage('Sexo é um campo obrigratório');
  end else
  if (DBECNH.Text = '') then
  Begin
    ShowMessage('CNH é um campo obrigratório');
    DBECNH.SetFocus;
  end else
  if (DBECEP.Text = '') then
  Begin
    ShowMessage('CEP é um campo obrigratório');
    pgcClientes.ActivePage := tsEndereco;
    DBECEP.SetFocus;
  end else
  if (DBENumero.Text = '') then
  Begin
    ShowMessage('Numero é um campo obrigratório');
    pgcClientes.ActivePage := tsEndereco;
    DBENumero.SetFocus;
  end else
...
begin
  ...
  Begin
    ShowMessage('Nome Fantasia é um campo obrigratório');
    DBENmFant.SetFocus;
  end else
  if (DBERzSocial.Text = '') then
  Begin
    ShowMessage('Razão Social é um campo obrigratório');
    DBERzSocial.SetFocus;
  end else
  if (DBECNPJ.Text = '') then
  Begin
    ShowMessage('CNPJ é um campo obrigratório');
    DBECNPJ.SetFocus;
  end else
  if (DBECEP.Text = '') then
  Begin
