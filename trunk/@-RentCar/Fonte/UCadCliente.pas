...
begin
  ...
  Begin
     application.messagebox(PChar('Cep '+Num+' não foi encontrado'),'Aviso',48);
  end else
  if (texto = 'CEP não está no formato 00000-000 ou 00000000') then
  Begin
    application.messagebox('CEP não está no formato 00000-000 ou 00000000','Aviso',48);
  end else
  Begin
...
begin
 ...
 Begin
   ShowMessage('Não existe(m) dado(s) para sere(m) excluido');
 end;
end;
...
begin
   ...
   Begin
     application.messagebox('O campo CPF deve ter no mínimo 11 numeros','Aviso',48);
     DBECPF.SetFocus;
   end else
   Begin
   cpf := DBECPF.Text;
   ...
   if lblResCPFCNPJ.Caption <> 'CPF Válido' then
    Begin
     ShowMessage('CPF Inválido, digite outro');
     DBECPF.SetFocus;
    end;
   end;
end;
...
begin
   ...
   Begin
     application.messagebox('O campo CNPJ deve ter no minino 11 números','Aviso',48);
     DBECNPJ.SetFocus;
   end else
   Begin
    cnpj := DBECNPJ.Text;
    ...
    Begin
     application.messagebox('CNPJ Inválido, digite outro','Aviso',48);
     DBECNPJ.SetFocus;
    end;
   end;
end;
...
{ dmRentCar.ZTPessoa.Open;
 ...
 Begin
   ShowMessage('Selecione o cliente que será excluido');
 end else
 Begin
 if (frmRentCarPrincipal.tipo = 'PF') or (frmRentCarPrincipal.tipo = 'FUN') then
 Begin
...
begin
  dias := YearsBetween(DateNascimento.date, Now);
   ...
   if (dias < 18) then
   Begin
     application.messagebox('A idade permitida é maior de 18 anos','Aviso',48);
     DateNascimento.SetFocus;
   end;
end;
